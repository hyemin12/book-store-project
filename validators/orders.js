...
const validatePostOrder = [
  ...
  body('delivery.recipient').trim().notEmpty().withMessage('받는 사람은 필수 입력'),
  body('delivery.address').trim().notEmpty().withMessage('주소는 필수 입력'),
  body('delivery.contact').trim().notEmpty().withMessage('전화번호는 필수 입력'),
  ...
  body('first_book_title').trim().notEmpty().withMessage('대표 도서는 필수 입력'),
  validateAndProceed
];
